def create_user(user_info) :
    ...
    for i in range (len(user_info)) :
        ...
        result = is_validation(user_info[i])
...
def is_validation(part_user_info) :

    is_wrong_data = True
    wrong_data = []
    result_list = []

    if part_user_info['blood_group'] not in blood_types :

        is_wrong_data = False
        wrong_data.append('blood_group')
    
    if '@' not in part_user_info['mail'] :

        is_wrong_data = False
        wrong_data.append('mail')

    if 2 < len(part_user_info['name']) and len(part_user_info['name']) > 30 :
                                                
        is_wrong_data = False
        wrong_data.append('name')

    if len(part_user_info['website']) < 1 :
    
        is_wrong_data = False
        wrong_data.append('website')

    if part_user_info['company'] in black_list :
    
        return 'blocked'
    
    if is_wrong_data == False :
...
print(len(create_user(user_data))
